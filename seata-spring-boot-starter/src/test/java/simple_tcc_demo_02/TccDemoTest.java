...
import simple_tcc_demo_02.config.TccConfig;
import simple_tcc_demo_02.service.EntryService;
...
class TccDemoTest {
    ...
    void setUp() {
        ...
        // 清理测试数据
        entryService.clearFrozenAmounts();
    ...
    void testTccSuccess() {
        ...
        // 执行 TCC 扣款
        boolean result = entryService.tryDeduct("1", 100);

        // 验证 Try 阶段成功
        assertTrue(result, "Try 阶段应该成功");

        // 检查冻结金额
        int frozenAmount = entryService.getFrozenAmount("1");
    ...
    void testTccRollback() {
        ...
        try {
            // 执行 TCC 扣款
            boolean result = entryService.tryDeduct("2", 100);
            assertTrue(result, "Try 阶段应该成功");

            // 检查冻结金额
            int frozenAmount = entryService.getFrozenAmount("2");
            assertEquals(100, frozenAmount, "冻结金额应该是 100");
            ...
            // 验证回滚后冻结金额被清理
            int frozenAmountAfterRollback = entryService.getFrozenAmount("2");
    ...
    void testTccInsufficientBalance() {
        ...
        // 尝试扣款超过余额的金额
        boolean result = entryService.tryDeduct("1", initialBalance + 100);
    ...
    void testTccAccountNotExists() {
        ...
        // 尝试扣款不存在的账户
        boolean result = entryService.tryDeduct("999", 100);

        // 验证 Try 阶段失败
        assertFalse(result, "账户不存在时 Try 阶段应该失败");

        // 验证没有冻结金额
        int frozenAmount = entryService.getFrozenAmount("999");
    ...
    void testTccIdempotency() {
        ...
        // 第一次执行 Try
        boolean result1 = entryService.tryDeduct("1", 100);
        assertTrue(result1, "第一次 Try 应该成功");

        // 第二次执行 Try（幂等性测试）
        boolean result2 = entryService.tryDeduct("1", 100);
